...
use App\Http\Requests\Academy\CourseStoreRequest;
use App\Http\Requests\Academy\CourseUpdateRequest;
use App\Http\Requests\Academy\CourseVerificationRequest;
...
class CourseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $courses = Course::query()
            ...
            ->withCount('reviews')
            ->withAvg('reviews', 'rating')
            ->when($user->role === 'student', fn($q) => $q->published())
            ...
            ->when($request->level, fn($q) => $q->where('level', $request->level))
            ->when($request->is_highlight !== null, fn($q) => $q->where('is_highlight', $request->boolean('is_highlight')))
            ->when($request->status && $user->role === 'admin', fn($q) => $q->where('status', $request->status))
            ->when($request->search, fn($q) => $q->where('title', 'like', '%' . $request->search . '%'))
            ->when($request->price_type === 'free', fn($q) => $q->free())
            ->when($request->price_type === 'paid', fn($q) => $q->paid())
            ->when($request->price_min, fn($q) => $q->where('price', '>=', $request->price_min))
    ...
    public function showPublic(Request $request, Course $course)
    {
